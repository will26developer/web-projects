window.addEventListener("DOMContentLoaded", () => {
    ...
    const resultsWrapCollection = document.querySelectorAll(".result__wrap");
    ...
    const initScoreStorage = () =>
        localStorage.setItem("scores", JSON.stringify({ player: 0, house: 0 }));
    ...
    const announceWinner = () => {
        if (playerScoreState >= 13 || houseScoreState >= 13) {
            ...
            message.textContent =
                playerScoreState >= 13 ? "Player win the game!" : "House win the game!";
        }
    };

    const showRules = () => rulesContainer.classList.add("active");

    const hideRules = () => rulesContainer.classList.remove("active");
    ...
    const botPlayer = () =>
        arrOption[Math.floor(Math.random() * arrOption.length)];
    ...
    const renderWinnerMessage = (winner, player, house) => {
        ...
        resultsWrapCollection.forEach((wrap) =>
            wrap.classList.remove("wrap__blue", "wrap__yellow", "wrap__red"),
        );
        message.textContent = messages[winner];
        pickedPlayer.src = `../../../main-projects/frontend-mentor/frontend-mentor-09/assets/img/images/icon-${player}.svg`;
        pickedHouse.src = `../../../main-projects/frontend-mentor/frontend-mentor-09/assets/img/images/icon-${house}.svg`;
